...
(async()=>{
 ...
 for(const lang of ['pl','en']){
  ...
  for(const model of ['ridge','elastic','huber','quantile']){
   ...
   if(model==='quantile'){
    for(const count of ['4','19','20']){await page.locator('#q-frequency').selectOption(count);for(const prediction of ['10','30']){await page.locator('[data-lab-value="'+prediction+'"]').click();const expected=prediction==='10'?19:count==='4'?4:count==='19'?19:20;check((await page.locator('.lab-readout').innerText()).includes(String(expected)),lang+' Q95 '+count+' '+prediction);}}
   }
 ...
 await browser.close();console.log(JSON.stringify({checks,errors,networkOrigins:[...new Set(requests.map(r=>new URL(r).origin))],directFile:true},null,2));
