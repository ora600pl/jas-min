...
(async()=>{
 ...
 for(const lang of ['pl','en']){
  ...
  for(const model of ['ridge','elastic','huber','quantile']){
   ...
   if(model==='elastic'){check(await page.locator('#intuition-slider').inputValue()==='4',lang+' Elastic primary lambda synchronized from maths');check((await page.locator('.fixed-observations').innerText()).includes(lang==='pl'?'10,6':'10.6'),lang+' maths control leaves observation fixed');}
