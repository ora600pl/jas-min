...
The mandatory analysis bootstrap teaches the model which statistical calculations and attachments are available, returns compact high-signal seeds, and creates an explicit analysis session. Subsequent calls retrieve focused evidence or relevant sections from `reasonings.txt`. Report tools maintain a stable eleven-section structure while allowing per-section detail and Markdown/JSON output preferences. Non-draft finalization requires findings in all nine analytical categories plus complete structured tables for gradient/anomaly synthesis, every unique plan-hash variant in every supplied execution-plan attachment, every supplied child-cursor diagnostic, every object in every non-empty segment-hotspot category, and the fixed performance-parameter checklist for every project. `get_report_status` returns the exact missing evidence and row keys, so completeness is a server gate rather than a prompt convention. Human-facing findings require a plain-language `evidence_summary` with exact values; raw evidence IDs remain structured provenance rather than reader-facing citations. Comparative prose must label each project or instance value explicitly instead of using an ambiguous `X/Y` shorthand, empty attachments must not be linked as reader-facing evidence, and alert counts must be scoped to observed attachment timestamps. Material wait-event names and SQL IDs must themselves be active links to every existing project-specific detail report; generic labels such as `instance 1` are not sufficient link text. A zero-match literal is not accepted as proof of absence until raw context and punctuation/message variants are checked. Applying diagnostic guidance requires a verbatim quotation that JAS-MIN verifies against the retrieved guidance text. If HTML is requested, the model finalizes the Markdown report first and calls `convert_markdown_to_html`; JAS-MIN accepts only the exact latest finalized Markdown, validates the structure, rejects unresolved report placeholders, and creates a new HTML file in its working directory with the same renderer used by classic AI mode. The renderer always imposes the same responsive ORA-600-aligned audit presentation: a self-contained vector JAS-MIN wordmark, white/black/red palette, sticky report navigation, severity-marked findings derived from `[severity / confidence]`, card layouts for executive findings and actions, styled evidence tables and execution plans, keyboard-visible link focus, mobile reflow, and a print stylesheet. Comparative HTML reports link to every selected project's main dashboard and load-profile pages. Classic navigation publishes verified links to existing source reports instead of depending on iframe placeholders.
