...
pub(crate) fn render(atlas: &SignalAtlas, linked: bool) -> String {
    ...
    out.push_str("<details class=\"signal-method\"><summary>How to read this analysis</summary><p><b>Active / peak:</b> source combined P90 / P99 impact across eligible model outputs. Percentiles include zeros: zero active impact does not rule out a peak, and P99 is not the maximum. Different target and predictor families are separate fits. Do not add their scores or treat a high score as saved seconds.</p><p><b>Model selection:</b> a filled marker reproduces source top-selection membership. Gradient v2 combines independently ranked active, peak and extreme selections; consult fit diagnostics and observation coverage. A dash is not a zero coefficient or proof of collinearity. A question mark means no selection evidence was supplied. Models share the same data; agreement is not independent causal proof.</p><p><b>Precision and scope:</b> displayed values are rounded; hover a number for the original value. Peak ÷ active is undefined at zero active impact and can be unstable near zero. Raw tables, exact values, method settings and provenance remain in the complete technical evidence below.</p></details></section>\n\n");
    out
}
