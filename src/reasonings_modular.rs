...
impl ChatClient {
    ...
    pub fn new(
        ...
        if use_openrouter {
            ...
            Ok(Self::Local(LocalOpenAiCompatClient::from_env(
                model_hint,
                temperature,
                max_tokens,
            )))
...
impl LocalOpenAiCompatClient {
    pub fn from_env(model_hint: &str, temperature: f64, max_tokens: usize) -> Self {
        ...
        let model = env::var("LOCAL_MODEL").unwrap_or_else(|_| model_hint.to_string());

        let api_key = env::var("LOCAL_API_KEY").unwrap_or("X".to_string());

        debug_note!(
            "Local client url: {} and key with len {} chars",
            base_url,
            api_key.len()
        );
    ...
    pub async fn chat_content(
        ...
        debug_note!(
            "Payload for local client created - asking for response from url {}",
            url
        );
...
impl OpenRouterClient {
    pub fn from_env(
        ...
        let api_key =
            env::var("OPENROUTER_API_KEY").map_err(|_| "Missing OPENROUTER_API_KEY env var")?;
    ...
    pub async fn chat_content(
        ...
        debug_note!(
            "Payload for OpenRouter client created - asking for response from url {}",
            url
        );
...
fn user_prompt_for_section(
    section: Section,
    capsule_json: &str,
    section_input_json: &str,
) -> String {
    match section {
        Section::Baseline => format!(
            r#"
...
"#
        ...
        Section::WaitEventsForeground => format!(
            r#"
Return SectionNotes JSON.
...
"#
        ...
        Section::WaitEventsBackground => format!(
            r#"
Return SectionNotes JSON.
...
"#
        ...
        Section::IoStatsByFunction => format!(
            r#"
Return SectionNotes JSON.
...
"#
        ...
        Section::SegmentsRowLockWaits => {
            segment_prompt("segments_row_lock_waits", capsule_json, section_input_json)
        }
        Section::SegmentsPhysicalWrites => {
            segment_prompt("segments_physical_writes", capsule_json, section_input_json)
        }
        Section::SegmentsPhysicalWriteRequests => segment_prompt(
            "segments_physical_write_requests",
            capsule_json,
            section_input_json,
        ),
        Section::SegmentsPhysicalReadRequests => segment_prompt(
            "segments_physical_read_requests",
            capsule_json,
            section_input_json,
        ),
        Section::SegmentsLogicalReads => {
            segment_prompt("segments_logical_reads", capsule_json, section_input_json)
        }
        Section::SegmentsDirectPhysicalWrites => segment_prompt(
            "segments_direct_physical_writes",
            capsule_json,
            section_input_json,
        ),
        Section::SegmentsDirectPhysicalReads => segment_prompt(
            "segments_direct_physical_reads",
            capsule_json,
            section_input_json,
        ),
        Section::SegmentsBufferBusyWaits => segment_prompt(
            "segments_buffer_busy_waits",
            capsule_json,
            section_input_json,
        ),

        //Gradient: same pattern, only data differs.
        Section::GradientWaitEvents => gradient_prompt(
            "db_time_gradient_fg_wait_events",
            capsule_json,
            section_input_json,
        ),
        Section::GradientStatsCounters => gradient_prompt(
            "db_time_gradient_instance_stats_counters",
            capsule_json,
            section_input_json,
        ),
        Section::GradientStatsVolume => gradient_prompt(
            "db_time_gradient_instance_stats_volumes",
            capsule_json,
            section_input_json,
        ),
        Section::GradientStatsTime => gradient_prompt(
            "db_time_gradient_instance_stats_time",
            capsule_json,
            section_input_json,
        ),
        Section::GradientSQLs => gradient_prompt(
            "db_time_gradient_sql_elapsed_time",
            capsule_json,
            section_input_json,
        ),

        Section::InstanceStatsCorrelation => format!(
            r#"
Return SectionNotes JSON.
...
"#
        ...
        Section::LoadProfileAnomalies => format!(
            r#"
Return SectionNotes JSON.
...
"#
        ...
        Section::AnomalyClusters => format!(
            r#"
Return SectionNotes JSON.
...
fn trim_foreground_waits(
    ...
    v.sort_by(|a, b| {
        b.avg_pct_of_dbtime
            .partial_cmp(&a.avg_pct_of_dbtime)
            .unwrap_or(Ordering::Equal)
    });
    v.truncate(waits_top_n);

    for e in &mut v {
        e.median_absolute_deviation_anomalies.sort_by(|a, b| {
            b.mad_score
                .partial_cmp(&a.mad_score)
                .unwrap_or(Ordering::Equal)
        });
...
fn trim_sqls(
    ...
    v.sort_by(|a, b| {
        b.avg_elapsed_time_cumulative_s
            .partial_cmp(&a.avg_elapsed_time_cumulative_s)
            .unwrap_or(Ordering::Equal)
    });
    v.truncate(sqls_top_n);

    for s in &mut v {
        s.median_absolute_deviation_anomalies.sort_by(|a, b| {
            b.mad_score
                .partial_cmp(&a.mad_score)
                .unwrap_or(Ordering::Equal)
        });
...
fn trim_load_profile_anomalies(
...
pub async fn analyze_report_modular_lmstudio(
    ...
    let baseline_notes =
        run_section(&client, &system, Section::Baseline, "", baseline_input).await?;

    let mut notes: Vec<(Section, String)> = vec![(Section::Baseline, baseline_notes)];

    // 1) Foreground waits (trimmed)
    let base_user_prompt_str =
        user_prompt_for_section(Section::WaitEventsForeground, &capsule_json, "{}");
    ...
    let base_user_prompt_str =
        user_prompt_for_section(Section::WaitEventsBackground, &capsule_json, "{}");
    ...
    // 3) SQLs (trimmed)
    let base_user_prompt_str =
        user_prompt_for_section(Section::SqlElapsedTime, &capsule_json, "{}");
    ...
    let base_user_prompt_str =
        user_prompt_for_section(Section::LoadProfileAnomalies, &capsule_json, "{}");
    ...
    let composer_user = user_prompt_for_section(Section::ComposeFinal, "", &bundle);

    let final_raw = client
        .chat_content(&composer_system, &composer_user)
        .await?;
